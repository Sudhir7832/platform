...
import { Zap, Mail, Lock, User, AlertCircle, ArrowRight, Sparkles } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Perform auth signup on Supabase
      ...
      // Check if email confirmation is required by Supabase config
      if (data?.session) {
        // Automatically logged in (email confirmation disabled in Supabase)
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-[#f0f0f5] flex font-sans overflow-hidden relative">
      ...
      {/* LEFT COLUMN: Premium SaaS Showcase Panel (Matched with Login) */}
      ...
      {/* RIGHT COLUMN: Interactive Registration Container */}
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div className="flex items-start gap-2.5 pt-1 select-none">
                <input
                  ...
                  disabled={loading}
                  className="w-4 h-4 mt-0.5 rounded bg-[#0d0d20] border-[rgba(124,58,237,0.15)] text-[#7c3aed] focus:ring-0 cursor-pointer"
                />
                <label htmlFor="terms" className="text-xs text-[#6b7280] font-medium leading-relaxed cursor-pointer">
                  I agree to the{' '}
                  <a href="#" className="text-[#a78bfa] hover:underline font-semibold">Terms of Service</a>{' '}
                  and{' '}
                  <a href="#" className="text-[#a78bfa] hover:underline font-semibold">Privacy Policy</a>
                </label>
              </div>
